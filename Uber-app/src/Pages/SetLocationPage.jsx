...
const SetLocationPage = () => {
  ...
  const handleSelectt = ( userInput) => {
    console.log("User Input:", userInput);
    setToLocation(userInput);
  };
  ...
  return (
    <div className="p-4 w-full h-screen flex flex-col">
      ...
      <Link
        to="/savedplaces"
        className=" flex items-center gap-2 mt-5 px-4 py-2 border border-gray-300 rounded-md"
      >
        {" "}
        <HiGlobeAsiaAustralia size={30} />{" "}
        <span className="text-xl font-semibold">Saved Places</span>
      </Link>

      {fromLocation && toLocation ? (
        <Link
          onClick={handelConfirm}
          to={`/confirmlocationpage?from=${encodeURIComponent(
            cityName
